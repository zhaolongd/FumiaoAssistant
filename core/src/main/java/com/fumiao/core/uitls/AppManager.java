...
public class AppManager {
    ...
    private void backTo(int step) {
        if (activityList == null) {
            L.e(String.format("Failed to backTo(%s)", step));
            return;
        }
...
//            activities.get(activities.size()-1).onResult(new Bundle());
    ...
    public void backToTagFront(Object tag) {
        if (activityList == null) {
            L.e(String.format("Failed to backToTagFront()"));
            return;
        }
        int step = 0;
        synchronized (activityList) {
    ...
    public void backToTag(Object tag, Object bundle) {
        if (activityList == null) {
            L.e(String.format("Failed to backToTag()"));
            return;
        }
        int step = 0;
        synchronized (activityList) {
...
//            activities.get(activities.size()-1).onResult(new Bundle());
            ...
            ActivityManager activityManager = (ActivityManager) CoreApp.getSingle().getSystemService(Context.ACTIVITY_SERVICE);
            activityManager.restartPackage(CoreApp.getSingle().getPackageName());
            System.exit(0);
        }
    }
